...
Prevention: A default test configuration must not be kept green by narrowing collection. When same-named helper modules exist per suite, disambiguate import resolution explicitly (per-module cache reset or unique module names) and assert the default collection covers the union of important suites.
